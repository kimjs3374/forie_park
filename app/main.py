...
def index():
    ...
    shown = regs[:10]
    logs_map = models.visit_logs_by_regs([r.id for r in shown])
    now_utc = datetime.now(timezone.utc)
    summaries = {r.id: models.summarize_logs(logs_map.get(r.id, []), now=now_utc) for r in shown}
    dispmap = models.visit_dispmap(shown, now=now_utc, logs_map=logs_map)
    return render_template("main/index.html", registrations=shown,
                           overdue=overdue, popups=popups, dispmap=dispmap, summaries=summaries)
...
def visit_list():
    ...
    return render_template("main/visit_list.html", registrations=regs, dispmap=dispmap, summaries=summaries)
